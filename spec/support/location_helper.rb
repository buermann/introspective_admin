...
module LocationHelper
  def rand_coords
    # generate a random point somewhere around the test airport
    l = LocationGps.make
    [l.lat, l.lng, l.alt]
  end
  ...
  def create_test_airport
    ...
    @sprocket_co = Company.find_by_name('Sprockets') || Company.make(name: 'Sprockets')
    @widget_co   = Company.find_by_name('Widgets')   || Company.make(name: 'Widgets')
    return @airport if (@airport = Location.find_by_name('TEST'))

    @airport = Location.new(name: 'TEST', kind: 'airport')
    @airport.companies.push @sprocket_co
    @airport.companies.push @widget_co
    ...
    (1..8).each do |terminal|
      ...
      (1..3).each do |number|
        ...
        g.beacons.push LocationBeacon.make(company: @sprocket_co, location: g)
        g.beacons.push LocationBeacon.make(company: @widget_co, location: g)
